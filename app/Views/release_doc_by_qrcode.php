        ...
        <div class="card bg-light shadow mobile-fixed-full">
            ...
            <div class="card-body bg-black  rounded" style="overflow: hidden !important">
                <div class="row  d-flex flex-row justify-content-center align-content-center">
                    ...
                    <div class="col-12  d-flex flex-row justify-content-center align-content-center ">
                        <div class="form-group d-flex flex-column p-1 px-2 text-danger" style="position: absolute;top:0px; background-color: rgba(0,0,0,1);border-radius:0 0 10px 10px;">
                            Release Scan
                        </div>
...
<script>
    ...
    function extractDocumentCode(url) {
        ...
        if (String(code).trim().length !== 0) {
            if (!hasSounded) {
                generateTone(3540, 200); // frequency in Hz, duration in milliseconds
            }
            hasSounded = true;

            //generateTone(2040, 200); // frequency in Hz, duration in milliseconds
            window.location.href = "<?= base_url() ?>confirm_release_qrcode_data/" + code;
